...
# Loading SVM model
print("Loading model from: {}".format(MODEL_PATH_SVM))
inference_svm = load(MODEL_PATH_SVM)

# Creation of the Flask app
app = Flask(__name__)

# API 1
# Flask route to serve HTTP traffic on that route
@app.route('/line/<Line>')
...
# Return prediction for LDA, Neural Network, and SVM models with the requested row as input
def prediction(Line):
    ...
    return {
        ...
        'prediction SVM': int(prediction_svm)
    }

# API 3
# Flask route to serve HTTP traffic on that route
@app.route('/score',methods=['POST', 'GET'])
# Return classification score for LDA, Neural Network, and SVM models from the entire dataset provided
def score():
    data = pd.read_json('./test.json')
    data_test = data.transpose()
